...
void main() {
  ...
  group('MediaService Tests', () {
    test('Create note directories through public API', () async {
      ...
      await mediaService.saveImage(noteId, 'test_image.jpg', testImageFile.path);
      ...
      await mediaService.saveVideo(noteId, 'test_video.mp4', testVideoFile.path);
      ...
      await mediaService.saveAudio(noteId, 'test_audio.mp3', testAudioFile.path);
    ...
    test('Save and retrieve media files', () async {
      ...
      final savedPath = await mediaService.saveImage(noteId, 'test_image.jpg', testFile.path);
      ...
      await Directory(path.join(tempDir.path, 'images', noteId)).delete(recursive: true);
    });
    ...
    test('Get all media for a note', () async {
      ...
      await Directory(path.join(tempDir.path, 'images', noteId)).delete(recursive: true);
      await Directory(path.join(tempDir.path, 'videos', noteId)).delete(recursive: true);
      await Directory(path.join(tempDir.path, 'audios', noteId)).delete(recursive: true);
    });
